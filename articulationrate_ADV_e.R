...
ARTdata_s %>%
  ggplot(aes(x=group, y=rate, fill=sex)) + geom_boxplot(alpha=0.5) + ylab("Articulation rate relative to QUIET") + xlab("Age group") + theme_bw(base_size = 16) +
  scale_fill_manual(values = c("#999999", "#FFFFFF")) + scale_y_continuous(breaks=seq(-2, 2, by =0.5), limits = c(-1.5,1)) + geom_hline(yintercept = 0.0, linetype = "dashed") + theme (legend.position = c(0.9, 0.87))
