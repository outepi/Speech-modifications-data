...
group_by(sex,group) %>% 
mutate(sex_count = n()) %>% 
ggplot(aes(x=group, y=f0, fill=sex)) + 
geom_boxplot(data = . %>% filter(sex_count >= min_n), outlier.shape = NA) +  
geom_jitter(data = . %>% filter(sex_count < min_n))+ 
ylab("Median f0 (semitones re 1 Hz)") + xlab("Age group") + theme_bw(base_size = 16) +
scale_fill_manual(values = c("#999999", "#FFFFFF")) + scale_y_continuous(breaks=seq(72, 108, by = 6)) + theme (legend.position = c(0.9, 0.87))
