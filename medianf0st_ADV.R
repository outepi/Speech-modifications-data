...
F0data_s %>%
  ggplot(aes(x=group, y=f0, fill=condition)) + geom_boxplot(alpha=0.5) + ylab("Median f0 relative to NORM") + xlab("Age group") + theme_bw(base_size = 16) +
  scale_fill_manual(values = c("#737CA1", "#E77471")) + scale_y_continuous(breaks=seq(-1, 8, by = 2), limits = c(-1.0,8)) + geom_hline(yintercept = 0.0, linetype = "dashed") + theme (legend.position = "none")
